...
import com.twelvet.websocket.netty.annotation.WebSocketEndpoint;
import com.twelvet.websocket.netty.autoconfigure.annotation.EnableWebSocket;
...
import com.twelvet.websocket.netty.domain.WebSocketMethodMapping;
...
public class WebSocketEndpointExporter extends ApplicationObjectSupport
    ...
    private void scanPackage(ApplicationContext applicationContext) {
        ...
        // All EnableWebSocket annotation beans
        Map<String, Object> enableWebSocketBeans = applicationContext.getBeansWithAnnotation(EnableWebSocket.class);
        for (Object value : enableWebSocketBeans.values()) {
            EnableWebSocket enableWebSocket = AnnotationUtils.findAnnotation(value.getClass(), EnableWebSocket.class);
            assert enableWebSocket != null;
            // Only take the first scan path
    ...
    private void registerEndpoint(Class<?> endpointClass) {
        WebSocketEndpoint annotation = AnnotatedElementUtils.findMergedAnnotation(endpointClass, WebSocketEndpoint.class);
        if (annotation == null) {
