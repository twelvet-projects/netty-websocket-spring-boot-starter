...
/**
 * @author twelvet Process request information
 */
public class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
	...
	public HttpServerHandler(WebSocketEndpointServer webSocketEndpointServer,
	...
	private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
		...
		if (!ObjectUtils.isEmpty(webSocketEndpointServer.getHost())
				&& !webSocketEndpointServer.getHost().equals("0.0.0.0")
				&& !webSocketEndpointServer.getHost().equals(host.split(":")[0])) {
			if (forbiddenByteBuf != null) {
				res = new DefaultFullHttpResponse(HTTP_1_1, FORBIDDEN, forbiddenByteBuf.retainedDuplicate());
			}
			else {
