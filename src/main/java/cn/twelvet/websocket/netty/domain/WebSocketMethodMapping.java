...
import cn.twelvet.websocket.netty.support.impl.*;
...
public class WebSocketMethodMapping {
	...
	public WebSocketMethodMapping(Class<?> webSocketClazz, ApplicationContext context, AbstractBeanFactory beanFactory)
			...
			for (Method method : currentClazzMethods) {
				if (method.getAnnotation(BeforeHandshake.class) != null) {
					...
					if (handshake == null) {
						...
						if (currentClazz == webSocketClazz || !isMethodOverride(handshake, method)) {
					...
					if (open == null) {
						...
						if (currentClazz == webSocketClazz || !isMethodOverride(open, method)) {
							// Duplicate annotation
					...
					if (close == null) {
						...
						if (currentClazz == webSocketClazz || !isMethodOverride(close, method)) {
							// Duplicate annotation
					...
					if (error == null) {
						...
						if (currentClazz == webSocketClazz || !isMethodOverride(error, method)) {
							// Duplicate annotation
					...
					if (message == null) {
						...
						if (currentClazz == webSocketClazz || !isMethodOverride(message, method)) {
							// Duplicate annotation
					...
					if (binary == null) {
						...
						if (currentClazz == webSocketClazz || !isMethodOverride(binary, method)) {
							// Duplicate annotation
					...
					if (event == null) {
						...
						if (currentClazz == webSocketClazz || !isMethodOverride(event, method)) {
							// Duplicate annotation
		...
		// When the method is rewritten and there is no annotation, the parent annotation
		// function of this method will be cancelled
	...
	private boolean isOverrideWithoutAnnotation(Method[] methods, Method superClazzMethod,
		...
		for (Method method : methods) {
			// Whether the method is overridden and whether the obtained annotation is
			// empty
